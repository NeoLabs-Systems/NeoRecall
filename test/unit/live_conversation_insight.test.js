...
function migratedDatabase() {
  ...
  db.exec(`
    ...
    INSERT INTO ai_requests (id,user_id,purpose,provider,model,state,reserved_at)
      VALUES ('request-1','user-1','consolidation','llama','test/model','succeeded','2026-07-13T10:00:00Z'),
             ('request-2','user-1','ask','llama','test/model','succeeded','2026-07-13T10:05:00Z');
...
test('the rebuild widens the request purpose without dropping requests or their references', () => {
  ...
  db.prepare(`INSERT INTO ai_requests (id,user_id,purpose,provider,model,state,reserved_at)
    VALUES ('request-3','user-1','conversation_preview','llama','test/model','sent','2026-07-13T10:10:00Z')`).run();
  assert.throws(() => db.prepare(`INSERT INTO ai_requests (id,user_id,purpose,provider,model,state,reserved_at)
    VALUES ('request-4','user-1','something_else','llama','test/model','sent','2026-07-13T10:11:00Z')`).run(), /CHECK constraint/);
  db.close();
});
