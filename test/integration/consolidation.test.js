...
test('one outbound request creates English memories and a durable interval gate', async () => {
  ...
  const segmentPublicId = crypto.randomUUID(); const conversationId = crypto.randomUUID();
  server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      conversationAssessments: [{ conversationId: 'c1', memoryWorthy: true }], entities: [{ ref: 'person-1', kind: 'person', canonicalNameEn: 'Alex', displayName: 'Alex', aliases: [] }],
      memories: [{ type: 'project_discussion', titleEn: 'Project planning', summaryEn: 'Alex agreed to prepare the release plan.', importance: 8,
        startedAt: '2099-01-01T00:00:00.000Z', endedAt: '2099-01-01T01:00:00.000Z', sourceConversationIds: ['c1'], sourceSegmentIds: ['s1'],
      ...
      dailySummary: { localDate: '2026-07-13', timezone: 'UTC', summaryEn: 'The release plan was discussed and assigned to Alex.', coverageStartedAt: '2026-07-13T10:00:00.000Z', coverageEndedAt: '2026-07-13T10:05:00.000Z', sourceCount: 1 },
      }) } }] }));
    });
  });
  ...
  db.prepare("INSERT INTO transcript_segments(public_id,user_id,chunk_id,conversation_id,source_component,started_at,ended_at,chunk_start_ms,chunk_end_ms,text,language) VALUES (?,?,?,?, 'combined',?,?,0,300000,?,'de')")
    .run(segmentPublicId, userId, chunk, conversationId, '2026-07-13T10:00:00.000Z', '2026-07-13T10:05:00.000Z', 'Alex übernimmt den Release-Plan.');
  ...
  assert.deepEqual(outboundBody.response_format.json_schema.schema.properties.conversationAssessments.items.properties.conversationId.enum, ['c1']);
  assert.equal(outboundBody.response_format.json_schema.schema.properties.conversationAssessments.minItems, 1);
  assert.equal(outboundBody.response_format.json_schema.schema.properties.conversationAssessments.maxItems, 1);
  ...
  assert.deepEqual(storedMemory, { title_en: 'Project planning', started_at: '2026-07-13T10:00:00.000Z', ended_at: '2026-07-13T10:05:00.000Z' });
