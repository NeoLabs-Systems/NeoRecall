...
function installBrowserDependencies() {
  heading('Browsers');
  // The meeting bot needs REAL Google Chrome for reliable joins — Google Meet
  // detects Playwright's bundled Chromium and shows "You can't join this video
  // call". Install Chrome cross-platform via Playwright; keep Chromium as a
  // fallback. Both are managed by Playwright (no system-wide install).
  try {
    runOrThrow('npx', ['playwright', 'install', 'chrome'], { env: withInstallEnv() });
    logOk('Google Chrome installed (meeting bot)');
  } catch (error) {
    logWarn(`Could not install Google Chrome (${error.message}); meeting bot will fall back to Chromium`);
  }
  try {
    runOrThrow('npx', ['playwright', 'install', 'chromium'], { env: withInstallEnv() });
    logOk('Chromium installed');
  } catch (error) {
    logWarn(`Failed to install Chromium: ${error.message}`);
  }
}
...
function ensureModelsReady({ skipModels = false } = {}) {
  ...
  return models.downloadAll(({ file, received, total }) => {
    ...
    if (!require('../server/ai/provider_registry').ready()) {
      throw new Error('The language model is not available after model setup.');
    }
    require('../server/db/database').getDatabase().prepare('SELECT vec_version()').get();
    logOk('Models verified');
  });
}
...
function printHelp() {
  ...
  row('install', 'Install service, bootstrap runtime, and download the local speech and language models');
  row('setup [--skip-models]', 'Prepare models, database, and web client');
