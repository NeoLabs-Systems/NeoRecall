...
async function setup({ skipModels = false } = {}) {
  ...
  const client = webUi.ensureWebUi();
  process.stdout.write(client.built
    ? `Built NeoRecall web UI at ${client.path}.\n`
    : `NeoRecall web UI ready at ${client.path}.\n`);
...
function start() {
  const client = webUi.ensureWebUi();
  if (client.built) process.stdout.write(`Built NeoRecall web UI at ${client.path}.\n`);
...
function status() {
  if (!webUi.isWebUiReady()) {
    process.stdout.write(`${webUi.missingWebUiMessage()}\n`);
    process.exitCode = 1;
  }
