...
function ensureModelsReady({ skipModels = false } = {}) {
  ...
  return models.downloadAll(({ file, received, total }) => {
    ...
    // Speech detection and diarization are what NeoRecall still runs on the
    // audio itself. Their native runtime has no build for every platform, and
    // that is survivable — transcripts simply arrive without speaker identity —
