...
// Reads, and on first use creates, the administrator API key.
//
// The desktop app configures transcription and language-model providers through
// the admin API, so it needs a credential that exists before anyone has signed
// in anywhere. This key is that credential: it lives only in ~/.neorecall/.env
// (0600) and is accepted as a bearer token by the admin routes.
function cmdAdminKey(args = []) {
  const json = args.includes('--json');
  if (!json) heading('Administrator API Key');
  ensureDefaultEnvFile();
  const existing = parseEnv(readEnvFileRaw(ENV_FILE)).get('ADMIN_API_KEY');
  const key = existing && existing.trim()
    ? existing.trim()
    : crypto.randomBytes(32).toString('base64url');
  if (key !== existing) {
    upsertEnvValue(ENV_FILE, 'ADMIN_API_KEY', key);
    process.env.ADMIN_API_KEY = key;
  }
  if (json) console.log(JSON.stringify({ adminApiKey: key, created: key !== existing }));
  else {
    console.log(`  ${key}`);
    if (key !== existing) logOk(`Stored ADMIN_API_KEY in ${ENV_FILE}`);
  }
  return key;
}
...
function printHelp() {
  ...
  row('admin-key [--json]', 'Print the administrator API key, creating one if needed');
