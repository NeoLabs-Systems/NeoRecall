...
const manifestFiles = new Map(manifest.models.flatMap((model) => model.files.map((file) => [file.path, file])));

/// Whether an installed model file is the one this build pins.
///
/// Existence is not the same question, and the difference is not theoretical.
/// When the pinned speech model changed, a machine that had the previous
/// generation on disk kept answering "ready": the filenames were the same, so
/// every existence check passed, and the recogniser then loaded a transducer
/// encoder through the Whisper code path and aborted on every chunk. The server
/// looked healthy and transcribed nothing, indefinitely.
///
/// Size is compared rather than SHA-256 because this runs on the readiness path:
/// hashing two gigabytes on every probe is not affordable, while `statSync` is
/// free and separates "the file the manifest describes" from "a file with the
/// right name" — which is the failure that actually happened. `verifyAll` still
/// does the cryptographic check where it belongs, during setup.
///
/// A path the manifest does not describe is an operator's own file and is
/// accepted on existence alone; NeoRecall has no expectation to compare it to.
function installedMatchesManifest(relativePath, absolutePath) {
  if (!fs.existsSync(absolutePath)) return false;
  const expected = manifestFiles.get(relativePath);
  if (!expected) return true;
  return fs.statSync(absolutePath).size === expected.size;
}
...
module.exports = { manifest, downloadFile, downloadAll, pruneRetired, verifyAll, fileSha256, installedMatchesManifest };
