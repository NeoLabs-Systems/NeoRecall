...
void main() {
  testWidgets('speech that runs on is one passage, not one card per sentence', (
    ...
    expect(
      find.text('Wir fangen mit dem Zeitplan an. Der Termin bleibt im September.'),
      findsOneWidget,
    );
    expect(find.text('Danach die Aufgaben im Team.'), findsOneWidget);
    // Two passages fit under the cap, so there is nothing to expand.
    expect(find.byKey(const ValueKey<String>('transcript-excerpts-toggle')), findsNothing);
  });

  testWidgets('only the first passages are shown until the reader asks for the rest', (
    tester,
  ) async {
    ...
    await tester.tap(find.byKey(const ValueKey<String>('transcript-excerpts-toggle')));
  ...
  testWidgets('rows without usable text or timestamps do not break the list', (
    ...
    await tester.pumpWidget(
      wrap(
        TranscriptExcerpts(
          sources: <Map<String, dynamic>>[
            <String, dynamic>{'started_at': null, 'ended_at': null, 'text': 'Ohne Zeitstempel.'},
