...
void main() {
  ...
  group('finding an appliance', () {
    ...
    test(
      'recognises one by name when the platform hides service uuids',
      () async {
  ...
  group('connecting', () {
    ...
    test('reads the current state immediately instead of waiting', () async {
      rig.transport.statusOnRead = cborEncode(
        status(<String, Object?>{'st': 'recording', 'el': 5000}),
      );
  ...
  group('live status', () {
    test('follows the appliance as it changes', () async {
      ...
      rig.transport.pushStatus(
        status(<String, Object?>{'st': 'recording', 'el': 61000}),
      );
      await Future<void>.delayed(Duration.zero);

      expect(rig.controller.status!.isRecording, isTrue);
      expect(
        rig.controller.status!.recordingElapsed,
  ...
  group('commands', () {
    ...
    test('headphone actions carry the address', () async {
      ...
      expect(rig.lastCommand(), <String, Object?>{
        'c': 'bt_connect',
        'a': 'AA:BB:CC:DD:EE:FF',
      });
    });
    ...
    test(
      'a failed write becomes a sentence, never an exception on screen',
      () async {
        await rig.connect();
  ...
  group('scanning from the appliance', () {
    test(
      'networks arrive strongest first, as the appliance ordered them',
      () async {
        await rig.connect();
        ...
        expect(rig.controller.networks.map((WifiNetwork n) => n.ssid), <String>[
          'Kitchen',
          'Shed',
        ]);
      },
  ...
  group('results that do not fit one notification', () {
    ...
    test('a paged self-test is reassembled before anyone sees it', () async {
      ...
      expect(rig.controller.checks.map((ApplianceCheck c) => c.name), <String>[
        'codec driver',
        'usb gadget',
        'speakers',
      ]);
    ...
    test(
      'an appliance that sends no page numbers is still understood',
      () async {
    ...
    test(
      'a dropped page abandons the result rather than shortening it',
      () async {
        await rig.connect();
        await rig.controller.lookForHeadphones();
    ...
    test(
      'a second scan replaces the first rather than merging into it',
      () async {
        await rig.connect();
        await rig.controller.lookForHeadphones();
  ...
  group('setup', () {
    test('mints a key and hands the appliance everything at once', () async {
      ...
      final sent =
          cborDecode(rig.transport.provisionWrites.single)
    ...
    test(
      'the device is remembered and reached again on the next launch',
      () async {
    ...
    test('a failure never shows the owner an exception', () async {
      ...
      rig.transport.connectError = TimeoutException(
    ...
    test(
      'a wrong Wi-Fi password comes back in the appliance’s own words',
      () async {
        await rig.connect(pair: true);
        await rig.controller.completeSetup(
          wifiSsid: 'Kitchen',
    ...
    test(
      'success is the appliance confirming it, not the app assuming it',
      () async {
        await rig.connect(pair: true);
        await rig.controller.completeSetup(
          wifiSsid: 'Kitchen',
        ...
        expect(
          rig.controller.boundDeviceId,
          '44444444-4444-4444-8444-444444444444',
        );
        expect(
          rig.controller.isBoundTo('44444444-4444-4444-8444-444444444444'),
          isTrue,
        );
      },
    );
