...
Future<void> pumpAsk(WidgetTester tester, NeoRecallController controller) async {
    ...
    MaterialApp(
      ...
      home: Scaffold(
        body: ListenableBuilder(
          ...
          builder: (BuildContext context, _) => AskScreen(controller: controller),
        ),
      ),
    ),
  );
...
void main() {
  testWidgets('an unanswered question shows the question and the thinking indicator', (tester) async {
    final controller = NeoRecallController();
    addTearDown(controller.dispose);
    controller.askTurns = <AskTurn>[AskTurn(question: 'What did I do today?')];
  ...
  testWidgets('an answer replaces the indicator and ranks its sources', (tester) async {
    final controller = NeoRecallController();
    addTearDown(controller.dispose);
  ...
  testWidgets('a failed question keeps the conversation and says what happened', (tester) async {
    final controller = NeoRecallController();
    addTearDown(controller.dispose);
    controller.askTurns = <AskTurn>[
      AskTurn(question: 'Who was at the standup?')..error = 'That question could not be answered just now.',
    ];
    await pumpAsk(tester, controller);
    await tester.pumpAndSettle();
