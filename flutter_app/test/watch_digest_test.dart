...
void main() {
  ...
  test('signed-out phones send a digest that says so and nothing else', () {
    final digest = publisher.build(
      snapshot: HomeWidgetSnapshot.signedOut,
      now: now,
    );
  ...
  test('the count of what was left behind comes from the whole conversation', () {
    final digest = publisher.build(
      snapshot: snapshot(),
      now: now,
  ...
  test('an already-opened transcript is preferred over the timeline preview', () {
    final digest = publisher.build(
      snapshot: snapshot(),
      now: now,
  ...
  test('a conversation with no write-up yet says it is still being written', () {
    final digest = publisher.build(
      snapshot: snapshot(),
      now: now,
  ...
  test('an oversized digest is trimmed until it fits the Data Layer budget', () {
    final digest = publisher.build(
    ...
    expect(utf8.encode(payload).length, lessThanOrEqualTo(WatchDigest.maxPayloadBytes));
