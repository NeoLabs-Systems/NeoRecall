...
Use dueAt for an explicit action-item deadline. Always use null for occurredAt. Use open for status.
...
This request is one bounded pass: return at most ${MEMORY_MAX_COUNT} memories, at most ${MINI_MEMORY_MAX_COUNT} action items per memory, and at most ${ENTITY_MAX_COUNT} entities. These are upper bounds, not quotas. When more genuine action items exist than fit, keep the concrete commitments with clear owners and deadlines. Conversation sections are not limited: partition the whole input however many sections it takes.
