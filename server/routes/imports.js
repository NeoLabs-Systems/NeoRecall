...
router.post('/', validate(z.object({ id: z.string().uuid().optional(), deviceId: z.string().uuid().optional(), originalName: z.string().min(1).max(512),
  ...
  (req, res) => { res.status(201).json(service.declare(req.auth.userId, { ...req.body, sha256: req.body.sha256.toLowerCase() })); });
router.get('/:id', (req, res) => { res.json(service.get(req.auth.userId, req.params.id)); });
...
router.delete('/:id', (req, res) => { service.cancel(req.auth.userId, req.params.id); res.status(204).end(); });
