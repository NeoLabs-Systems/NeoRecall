...
function shell(title, eyebrow, content) {
  ...
  :root{color-scheme:dark;background:#0a0d13;color:#f4f3ee;font-family:Geist,Inter,ui-sans-serif,system-ui,sans-serif}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;padding:24px;background:radial-gradient(circle at 12% 10%,rgba(217,138,166,.13),transparent 34%),radial-gradient(circle at 90% 90%,rgba(227,182,85,.12),transparent 38%),#0a0d13}.card{width:min(560px,100%);padding:30px;border:1px solid rgba(255,255,255,.1);border-radius:24px;background:rgba(18,22,31,.92);box-shadow:0 28px 90px rgba(0,0,0,.38)}.eyebrow{color:#e3b655;font-size:12px;font-weight:800;letter-spacing:.16em}.brand{display:flex;align-items:center;gap:12px;margin-bottom:28px}.mark{width:38px;height:38px;display:grid;place-items:center;border-radius:12px;background:rgba(217,138,166,.14);border:1px solid rgba(217,138,166,.35);color:#d98aa6;font-weight:900}.brand strong{font-size:18px}h1{margin:8px 0 10px;font-size:30px;line-height:1.1}p{color:#afb5c2;line-height:1.55}.field{margin-top:14px}label{display:block;margin-bottom:7px;color:#d7dbe3;font-size:13px;font-weight:700}input{width:100%;padding:13px 14px;border:1px solid rgba(255,255,255,.12);border-radius:12px;background:#0e121a;color:#f4f3ee;font:inherit;outline:none}input:focus{border-color:#e3b655;box-shadow:0 0 0 3px rgba(227,182,85,.12)}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:22px}button{padding:12px 16px;border:0;border-radius:12px;font:inherit;font-weight:800;cursor:pointer}.primary{background:#e3b655;color:#17130a}.secondary{background:#252b37;color:#f4f3ee}.error{padding:11px 13px;margin-top:16px;border-radius:12px;background:rgba(222,95,112,.12);border:1px solid rgba(222,95,112,.35);color:#ffb5bf}.scope{display:flex;gap:11px;align-items:flex-start;padding:12px 0;border-top:1px solid rgba(255,255,255,.08)}.scope:first-child{border-top:0}.scope i{width:8px;height:8px;margin-top:7px;border-radius:50%;background:#d98aa6}.scope b{display:block}.scope span{display:block;color:#9fa7b5;font-size:13px;margin-top:3px}.redirect{padding:12px;border-radius:12px;background:#0e121a;color:#9fa7b5;font:12px ui-monospace,SFMono-Regular,monospace;overflow-wrap:anywhere}</style></head><body><main class="card"><div class="brand"><div class="mark">R</div><div><div class="eyebrow">${escapeHtml(eyebrow)}</div><strong>NeoRecall</strong></div></div>${content}</main></body></html>`;
}

function renderSignIn(continuePath, error = '') {
  return shell('Sign in', 'NEOAGENT CONNECTION', `<h1>Sign in to approve access</h1><p>Use your NeoRecall account. Your password is verified by this NeoRecall server and is never sent to NeoAgent.</p>${error ? `<div class="error">${escapeHtml(error)}</div>` : ''}<form method="post" action="/oauth/sign-in"><input type="hidden" name="continue" value="${escapeHtml(continuePath)}"><div class="field"><label for="account">Username or email</label><input id="account" name="account" autocomplete="username" required autofocus></div><div class="field"><label for="password">Password</label><input id="password" name="password" type="password" autocomplete="current-password" required></div><div class="field"><label for="two_factor_code">Two-factor code <span style="color:#7f8795;font-weight:400">(if enabled)</span></label><input id="two_factor_code" name="two_factor_code" inputmode="numeric" autocomplete="one-time-code"></div><div class="actions"><button class="primary" type="submit">Sign in securely</button></div></form>`);
}
