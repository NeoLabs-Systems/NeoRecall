...
const backups = require('../backup/backup_service');
...
function tick() {
  ...
  // Backups run off the same tick rather than their own timer so a host that was
  // asleep at the scheduled hour still backs up as soon as it wakes. `due` owns
  // the interval rule; the hour bucket keeps a minute-by-minute tick from
  // enqueueing the same run repeatedly.
  try {
    if (backups.due(db)) {
